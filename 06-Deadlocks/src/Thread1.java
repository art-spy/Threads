...
public class Thread1 extends Thread {
    ...
    public void run() {
        synchronized (lock1) {
            System.out.println("Thread 1: Has acquired lock-1 on object-1");
            try {Thread.sleep(500);}
            catch(InterruptedException e) {}

            System.out.println("Thread 1: Waiting for acquiring  lock-2 on object-2");
            try {Thread.sleep(500);}
            catch(InterruptedException e) {}

            synchronized (lock2) {
                System.out.println("Thread 1: Has acquired lock-1 on object-1 and lock-2 on object-2");
                try {Thread.sleep(500);}
                catch(InterruptedException e) {}
            }
            System.out.println("Thread 1: Released lock-2 from object-2");
            try {Thread.sleep(500);}
            catch(InterruptedException e) {}
        }
        System.out.println("Thread 1: Released lock-1 from object-2. Exiting...");
        try {Thread.sleep(500);}
        catch(InterruptedException e) {}
    }
}